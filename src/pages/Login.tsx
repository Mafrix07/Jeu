...
export const Login = () => {
  ...
  const { sessionId, playerName, startSession, loading } = useSession();
      ...
      <div className="w-full max-w-[400px]">
        ...
        <div className="grid grid-cols-1 gap-4">
          {PLAYERS.map((name, i) => {
            ...
            // Taken = someone already joined with this name, but we allow resuming
            return (
              <motion.button
                key={name}
                ...
                whileHover={!loading ? { x: 6, borderColor: 'var(--accent)' } : {}}
                whileTap={!loading ? { scale: 0.98 } : {}}
                onClick={() => !loading && handlePick(name)}
                disabled={loading}
                className="group p-6 rounded-2xl text-2xl font-black text-left border-2 flex justify-between items-center transition-all bg-[var(--card)] border-[var(--border)] text-white hover:text-[var(--accent)] disabled:opacity-60"
              >
                <div className="flex flex-col">
                  <span className="leading-none">{name}</span>
                  {isTaken
                    ? <span className="text-[10px] font-mono mt-1 text-[var(--accent)] uppercase tracking-wider">REPRENDRE →</span>
                    : <span className="text-[10px] font-mono mt-1 text-[var(--green)] uppercase tracking-wider">DISPONIBLE</span>
                  }
                </div>
